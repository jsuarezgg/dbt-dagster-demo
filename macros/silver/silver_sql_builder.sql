{%- macro silver_sql_builder(config_dict=none) %}
...
, union_all_events AS (
    ...
    WHERE silver.{{silver_table_name}}.ocurred_on_date BETWEEN (to_date('{{ var("start_date","placeholder_prev_exec_date") }}'- INTERVAL "{{var('incremental_slack_time_in_hours')}}" HOUR)) AND to_date('{{ var("end_date","placeholder_end_date") }}')
    UNION ALL
    {%- endif %}
    {%- endif %}
    {% for event_key, event_value in events_dict.items() -%}
