...
WITH syc_loan_status AS (
    ...
    SELECT * FROM {{ ref('d_syc_loan_status_co') }}

),
snc_payments_report AS (

    SELECT * FROM {{ ref('f_snc_payments_report_co') }}

),
syc_originations AS (

    SELECT * FROM {{ ref('f_syc_originations_co') }}
...
allies AS (
    ...
    SELECT * FROM {{ ref('d_ally_management_stores_allies_co') }}
