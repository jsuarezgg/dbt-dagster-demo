...
funnel_step_1 AS (
    ...
    {% if is_incremental() %}
        WHERE from_utc_timestamp(fst.ocurred_on, 'America/Bogota') > (SELECT MAX(ocurred_on_timestamp) FROM {{ this }})
    {% endif %}
),
...
funnel_step_3 AS (
    SELECT
		...
		CASE
			...
			WHEN journey_name IN ('BNPN_CO') THEN 'CO-BNPN'
			WHEN journey_name IN ('REFINANCE_CO') THEN 'CO-REFINANCE'
        ...
        CASE
			...
			WHEN journey_stage_name IN ('bnpn-summary-co') THEN 'bnpn-summary-co'
			...
			WHEN journey_stage_name IN ('pse-payment-co') THEN 'pse-payment-co'
			...
			WHEN journey_stage_name IN ('refinance-loan-proposal-selection-co') THEN 'refinance-loan-proposal-selection-co'
            WHEN journey_stage_name IN ('preconditions-refinance-pago-co') THEN 'preconditions-refinance-pago-co'
            WHEN journey_stage_name IN ('refinance-loan-acceptance-co') THEN 'refinance-loan-acceptance-co'
            WHEN journey_stage_name IN ('refinance-loan-proposals-co') THEN 'refinance-loan-proposals-co'
...
funnel_step_4 AS (
    SELECT
		...
		MAX(CASE WHEN journey_stage_homolog IN ('bnpn-summary-co') THEN flag_in ELSE 0 END) AS bnpn_summary_co_in,
		MAX(CASE WHEN journey_stage_homolog IN ('bnpn-summary-co') THEN flag_out ELSE 0 END) AS bnpn_summary_co_out,
        ...
        MAX(CASE WHEN journey_stage_homolog IN ('pse-payment-co') THEN flag_in ELSE 0 END) AS pse_payment_co_in,
		MAX(CASE WHEN journey_stage_homolog IN ('pse-payment-co') THEN flag_out ELSE 0 END) AS pse_payment_co_out,
		...
		MAX(CASE WHEN journey_stage_homolog IN ('refinance-loan-proposal-selection-co') THEN flag_in ELSE 0 END) AS refinance_loan_proposal_selection_co_in,
        MAX(CASE WHEN journey_stage_homolog IN ('refinance-loan-proposal-selection-co') THEN flag_out ELSE 0 END) AS refinance_loan_proposal_selection_co_out,
        MAX(CASE WHEN journey_stage_homolog IN ('preconditions-refinance-pago-co') THEN flag_in ELSE 0 END) AS preconditions_refinance_pago_co_in,
        MAX(CASE WHEN journey_stage_homolog IN ('preconditions-refinance-pago-co') THEN flag_out ELSE 0 END) AS preconditions_refinance_pago_co_out,
        MAX(CASE WHEN journey_stage_homolog IN ('refinance-loan-acceptance-co') THEN flag_in ELSE 0 END) AS refinance_loan_acceptance_co_in,
        MAX(CASE WHEN journey_stage_homolog IN ('refinance-loan-acceptance-co') THEN flag_out ELSE 0 END) AS refinance_loan_acceptance_co_out,
        MAX(CASE WHEN journey_stage_homolog IN ('refinance-loan-proposals-co') THEN flag_in ELSE 0 END) AS refinance_loan_proposals_co_in,
        MAX(CASE WHEN journey_stage_homolog IN ('refinance-loan-proposals-co') THEN flag_out ELSE 0 END) AS refinance_loan_proposals_co_out,
        MAX(flag_approval_event) AS flag_approval_event,
        MAX(flag_preapproval) AS flag_preapproval,
        MAX(privacy_policy_stage_in) AS privacy_policy_stage_in,
        MAX(privacy_first_name_co_out) AS privacy_first_name_co_out,
...
SELECT
    ...
    bnpn_summary_co_in,
    bnpn_summary_co_out,
    cellphone_validation_co_in,
    cellphone_validation_co_out,
    device_information_co_in,
    device_information_co_out,
    email_verification_co_in,
    email_verification_co_out,
    face_verification_co_in,
    face_verification_co_out,
    fraud_check_co_in,
    fraud_check_co_out,
    ...
    loan_acceptance_v2_co_in,
    loan_acceptance_v2_co_out,
    loan_proposals_co_in,
    loan_proposals_co_out,
    personal_information_co_in,
    personal_information_co_out,
    ...
    privacy_policy_v2_co_in,
    privacy_policy_v2_co_out,
    pse_payment_co_in,
    pse_payment_co_out,
    psychometric_assessment_co_in,
    psychometric_assessment_co_out,
    risk_evaluation_santander_co_in,
    risk_evaluation_santander_co_out,
    underwriting_co_in,
    underwriting_co_out,
    work_information_co_in,
    work_information_co_out,
    refinance_loan_proposal_selection_co_in,
    refinance_loan_proposal_selection_co_out,
    preconditions_refinance_pago_co_in,
    preconditions_refinance_pago_co_out,
    refinance_loan_acceptance_co_in,
    refinance_loan_acceptance_co_out,
    refinance_loan_proposals_co_in,
    refinance_loan_proposals_co_out
