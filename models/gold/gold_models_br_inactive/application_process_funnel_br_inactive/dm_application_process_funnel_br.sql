...
WITH applications_backfill_step_1 AS (
    ...
    FIRST_VALUE(fst.ally_slug, TRUE) AS ally_slug,
    FIRST_VALUE(fst.channel, TRUE) AS channel,
    FIRST_VALUE(fst.client_id, TRUE) AS client_id,
    FIRST_VALUE(fst.client_type, TRUE) AS client_type,
    FIRST_VALUE(fst.journey_name, TRUE) AS journey_name
    FROM {{ ref('f_origination_events_br_logs') }} fst
    GROUP BY 1
)
,
applications_backfill_step_2 AS (
	...
	LEFT JOIN (
		SELECT
	        ...
	        FROM {{ ref('f_origination_events_br_logs') }} fs2
	...
	LEFT JOIN {{ ref('f_applications_br') }} AS a 		ON fst.application_id = a.application_id
)
,
application_preapproval_br_filtered AS (
	...
	FROM {{ ref('f_applications_br') }} AS a
	INNER JOIN (
		SELECT
			application_id,
...
product_assignation as (
	select abs.application_id,
		   ...
		   case when orig.application_id is not null 
	...
	left join {{ ref('f_originations_bnpn_br') }} orig_bnpn
	on abs.application_id = orig_bnpn.application_id
)
,
funnel_step_1 AS (
  SELECT
	  ...
	  pa.term AS term,
	  CASE WHEN pa.synthetic_product_subcategory = 'CUPO Flex' AND pa.originated = 1 THEN 1 ELSE 0 END AS flag_flex
  FROM {{ ref('f_origination_events_br_logs') }} AS fst
...
funnel_step_2 AS (
	SELECT
		...
		MAX(fs1.flag_flex) AS flag_flex,
...
SELECT
	...
	ELEMENT_AT(ARRAY_SORT(fs4.applications_array, (LEFT, RIGHT) -> CASE
            ...
            WHEN LEFT.ocurred_on_timestamp > RIGHT.ocurred_on_timestamp THEN -1
            WHEN LEFT.ocurred_on_timestamp == RIGHT.ocurred_on_timestamp THEN 0
          	END), 1).synthetic_product_category as synthetic_product_category,
	ELEMENT_AT(ARRAY_SORT(fs4.applications_array, (LEFT, RIGHT) -> CASE
            WHEN LEFT.ocurred_on_timestamp < RIGHT.ocurred_on_timestamp THEN 1
            WHEN LEFT.ocurred_on_timestamp > RIGHT.ocurred_on_timestamp THEN -1
            WHEN LEFT.ocurred_on_timestamp == RIGHT.ocurred_on_timestamp THEN 0
          	END), 1).synthetic_product_subcategory as synthetic_product_subcategory,
	ELEMENT_AT(ARRAY_SORT(fs4.applications_array, (LEFT, RIGHT) -> CASE
            WHEN LEFT.ocurred_on_timestamp < RIGHT.ocurred_on_timestamp THEN 1
            WHEN LEFT.ocurred_on_timestamp > RIGHT.ocurred_on_timestamp THEN -1
            WHEN LEFT.ocurred_on_timestamp == RIGHT.ocurred_on_timestamp THEN 0
          	END), 1).flag_flex as flag_flex,
	ELEMENT_AT(ARRAY_SORT(fs4.applications_array, (LEFT, RIGHT) -> CASE
            WHEN LEFT.ocurred_on_timestamp < RIGHT.ocurred_on_timestamp THEN 1
            WHEN LEFT.ocurred_on_timestamp > RIGHT.ocurred_on_timestamp THEN -1
            WHEN LEFT.ocurred_on_timestamp == RIGHT.ocurred_on_timestamp THEN 0
