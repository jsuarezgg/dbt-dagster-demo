...
WITH syc_loan_status AS (
    ...
    SELECT * FROM {{ ref('d_syc_loan_status_br') }}

),
snc_payments_report AS (

    SELECT * FROM {{ ref('f_snc_payments_report_br') }}

),
syc_originations AS (

    SELECT * FROM {{ ref('f_syc_originations_br') }}

),
client_loans AS (

    SELECT * FROM {{ ref('f_fincore_loans_br') }}

),
dynamic_funnel_loan_proposals AS (

    SELECT * FROM {{ ref('f_loan_proposals_br') }}

),
originations AS (

    SELECT * FROM {{ ref('f_originations_bnpl_br') }}

),
applications AS (

    SELECT * FROM {{ ref('f_applications_br') }}

),
allies AS (

    SELECT * FROM {{ ref('d_ally_management_stores_allies_br') }}
...
pr_y_df AS (
    ...
    SELECT
        client_id,
        loan_id,
        sum(total_payment) AS total_pmt,
...
ls_df AS (
    ...
    SELECT
        ls_df_1.*,
        CAST(NULL AS string) AS refinancing_process_start_date,
        CAST(NULL AS string) AS refinancing_process_end_date,
        CAST(NULL AS string) AS refinancing_process_type,
        CAST(NULL AS string) AS refinancing_process_annulled,
        CAST(NULL AS string) AS refinancing_process_annullment_reason,
        CAST(NULL AS string) AS refinancing_order
    FROM ls_df_1
...
lpt_df AS (
    ...
    SELECT
        ...
        CAST(NULL AS string) AS total_fga_rate,
        CAST(NULL AS string) AS fga_comission_rate,
        CAST(NULL AS string) AS fga_client_rate,
        lpt.last_event_name_processed AS stage_loan_proposals_t
    FROM client_loans cld
    LEFT JOIN dynamic_funnel_loan_proposals lpt
    ON cld.loan_id = lpt.loan_proposal_id
...
apps_client_df AS (
    ...
    LEFT JOIN client_loans
        ON originations.loan_id = client_loans.loan_id
