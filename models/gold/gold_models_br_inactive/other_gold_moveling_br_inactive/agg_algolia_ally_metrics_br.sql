...
WITH br_applications AS (
    ...
    WHERE 1=1
        ...
        AND ocurred_on_date > date_add(current_date(),-{{days_before}})
    GROUP BY 1,2,3,4
)
, br_loans AS (
SELECT
	ally_slug,
	COUNT(loan_id) FILTER (WHERE o.origination_date > date_add(current_date(),-{{days_before}})) AS originations_orig,
	SUM(approved_amount) FILTER (WHERE o.origination_date > date_add(current_date(),-{{days_before}})) AS gmv,
	SUM(approved_amount) FILTER (WHERE o.origination_date > date_add(current_date(),-{{days_before}}))/ COUNT(loan_id) FILTER (WHERE o.origination_date > date_add(current_date(),-{{days_before}})) AS aov,
	SUM(CASE WHEN t.application_id IS NOT NULL THEN 1 ELSE 0 END) FILTER (WHERE o.origination_date > date_add(current_date(),-{{days_before}})) as app_originations,
	SUM(CASE WHEN t.application_id IS NOT NULL THEN approved_amount ELSE 0 END) FILTER (WHERE o.origination_date > date_add(current_date(),-{{days_before}})) as app_gmv,
	SUM(CASE WHEN t.application_id IS NOT NULL THEN approved_amount ELSE 0 END) FILTER (WHERE o.origination_date > date_add(current_date(),-{{days_before}})) 
	/ SUM(CASE WHEN t.application_id IS NOT NULL THEN 1 ELSE 0 END) FILTER (WHERE o.origination_date > date_add(current_date(),-{{days_before}})) AS app_aov,
...
LEFT JOIN {{ ref('f_marketplace_transaction_attributable_br') }} t
...
app_clicks AS (
    ...
    WHERE 1=1
        ...
        AND event_date > date_add(current_date(),-{{days_before}})
    GROUP BY 1
)
...
FROM {{ ref('dm_addi_okr_metrics_transactions_starting_addi_app') }} a
WHERE 1=1
...
SELECT
	ally_slug,
	'ScoreA (Orig. Last {{days_before}} days)' AS metric,
