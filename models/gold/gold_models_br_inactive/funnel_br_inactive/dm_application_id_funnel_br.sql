...
WITH applications_backfill_step_1 AS (
    ...
    {% if is_incremental() %}
        WHERE from_utc_timestamp(ocurred_on, 'America/Sao_Paulo') > (SELECT MAX(ocurred_on_timestamp) FROM {{ this }})
    {% endif %}
    GROUP BY 1
),
...
applications_backfill_step_2 AS (
    ...
    LEFT JOIN (
        ...
        WHERE journey_name IN ('CLIENT_PAGO_BR', 'PROSPECT_CHECKPOINT_PAGO_BR', 'PROSPECT_PAGO_BR', 'PROSPECT_PAGO_V2_BR')
            ...
            {% if is_incremental() %}
                AND from_utc_timestamp(ocurred_on, 'America/Sao_Paulo') > (SELECT MAX(ocurred_on_timestamp) FROM {{ this }})
            {% endif %}
    ) AS synthetic
...
{% if is_incremental() %}
    WHERE ocurred_on_timestamp > (SELECT MAX(ocurred_on_timestamp) FROM {{ this }})
{% endif %}
