...
with initial_table as (
  ...
  inner join {{ ref('f_originations_bnpl_br') }} orig_bnpl
    on apps.application_id = orig_bnpl.application_id
  left join {{ ref('dm_loan_status_br') }} ls
...
, dpd_w_dates as (
  ...
  left join {{ source('gold', 'dm_daily_loan_status_br') }} as dls
...
,condonation_by_mob as (
  ...
  from {{ ref('rmt_condonation_date_br') }} cd
