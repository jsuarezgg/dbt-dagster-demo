...
select a.application_id
      ...
      ,a.application_date
      ,a.campaign_id
      ,a.application_date as general_date
      ...
      ,lp.term
      ,a.ally_slug
      ,al.ally_name as ally_name
      ,al.vertical_name as ally_vertical
      ,a.client_type
      ,a.product
      --,a.application_product
      ,case when
    ...
    left join {{ ref('f_originations_bnpl_br') }} orig on a.application_id = orig.application_id
    left join {{ ref('f_originations_bnpn_br') }} orig_bnpn on a.application_id = orig_bnpn.application_id
    left join {{ ref('f_underwriting_fraud_stage_br') }} udw on a.application_id = udw.application_id
    left join {{ ref('d_ally_management_allies_br') }} al on a.ally_slug = al.ally_slug
    left join {{ ref('dm_loan_status_br') }} ls on orig.loan_id = ls.loan_id
    left join {{ ref('d_syc_clients_br') }} sc on a.client_id = sc.client_id
    left join (select a.loan_id,
                      row_number() over(partition by a.client_id order by a.origination_date) as loan_number
               from {{ ref('f_originations_bnpl_br') }} a
               where a.loan_id is not null) client_loan_number on orig.loan_id = client_loan_number.loan_id
    left join {{ ref('rmt_loan_fully_paid_date_br') }} fpd on orig.loan_id = fpd.loan_id
    left join {{ ref('f_loan_proposals_br') }} lp on orig.loan_id = lp.loan_proposal_id
    left join {{ ref('f_idv_stage_br') }} idv on a.application_id = idv.application_id
    left join {{ ref('f_idv_third_party_br') }} idv_3 on a.application_id = idv_3.application_id
    left join {{ ref('f_origination_events_br') }} orig_ev on a.application_id = orig_ev.application_id
    left join {{ ref('rmt_application_journey_stages_br') }} app_js on a.application_id = app_js.application_id
)
...
, consolidated as (
    select distinct
      ...
      ,from_utc_timestamp(apps.general_date, 'America/Sao_Paulo')::date as d_vintage --Application day
      ,trunc(from_utc_timestamp(apps.general_date, 'America/Sao_Paulo'),'week') as w_vintage --Application week
      ,date_format(from_utc_timestamp(apps.general_date, 'America/Sao_Paulo'), "yyyy-MM") as m_vintage --Application month
      ,date_format(from_utc_timestamp(apps.general_date, 'America/Sao_Paulo'),"yyyy-QQ") as q_vintage --Application quarter
      ,date_part('day',from_utc_timestamp(apps.general_date, 'America/Sao_Paulo')) as day_of_month
      ,apps.product
      ...
      ,cast(coalesce(pdm.preapproval_addi_pd, pdm.pd_no_multipliers, apps.addi_pd) as double) as addi_pd
      ...
      ,pdm.preapproval_credit_policy_name
      -- IDV Metrics 
      ,apps.bypass
      ,apps.possible_idv
      ,apps.idv_started
  ...
  ----<ECM variables>--
    ...
    from apps
    left join {{ ref('rmt_application_preapproval_br') }} app_pr
      ...
      and rn = 1
    left join {{ ref('rmt_first_preapproval_date_br') }} d on apps.prospect_id = d.client_id
    left join grouping_buckets g on apps.application_id = g.application_id
    left join {{ ref('dm_customer_segmentation') }} dm_cs on apps.prospect_id = dm_cs.client_id and dm_cs.country = 'BR'
    left join {{ ref('rmt_historic_idv_approved_br') }} hia on apps.prospect_id = hia.client_id 
    left join {{ ref('rmt_metrics_br') }} met on apps.loan_id = met.loan_id
    left join {{ ref('rmt_imply_risk_br') }} imp on from_utc_timestamp(apps.application_date, 'America/Sao_Paulo')::date = imp.day
                                     and apps.ally_slug = imp.ally_name 
                                     and apps.journey_name = imp.journey_name
                                     and apps.prospect_id = imp.client_id
    left join {{ ref('f_loan_cancellation_br') }} dflc on apps.loan_id = dflc.loan_id
    left join {{ ref('rmt_pd_models_br') }} pdm on apps.application_id = pdm.application_id
    left join {{ source('risk', 'seg2_cut_br') }} rsc on apps.prospect_id = rsc.client_id --backbook surgery
)
