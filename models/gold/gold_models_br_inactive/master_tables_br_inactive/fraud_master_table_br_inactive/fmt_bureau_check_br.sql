...
with prev_phones_emails as (
  ...
  left join {{ ref('f_pii_applications_br') }} a2
  ...
  left join {{ ref('f_idv_stage_br') }} idv
    on a.application_id = idv.application_id
  left join {{ ref('f_kyc_iovation_v1v2_br') }} iov
    on a.application_id = iov.application_id
  group by 1,2,3,4,5,6
)
select 
a.*,
case when instr(b.cellphones_string, SUBSTR(a.application_cellphone, 3)) > 0 then 1 else 0
...
left join {{ ref('fmt_kyc_all_info_br') }} b 
