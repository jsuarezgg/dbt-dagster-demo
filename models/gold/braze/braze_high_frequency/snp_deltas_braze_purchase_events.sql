...
{% set data_sources_config_dict = return_data_sources_config_braze('snp_braze_purchase_events') %}
SELECT
    snp.dbt_scd_id,
...
FROM {{ ref('snp_braze_purchase_events') }} AS snp
...
LEFT JOIN (
    ...
    WHERE snapshot_deltas_source = 'snp_deltas_braze_purchase_events'
    ) last_batch ON 1=1
{% endif %}
WHERE 1=1
