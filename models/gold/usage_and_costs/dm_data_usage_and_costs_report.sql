...
WITH base_metrics AS (
    ...
    SELECT
        ...
        MAX('{{ var("execution_date") }}') AS updated_at
    FROM {{ ref('f_amplitude_events_counts') }}
    GROUP BY 2
    ...
    SELECT
        ...
        MAX(NOW()) AS ingested_at,
        MAX('{{ var("execution_date") }}') AS updated_at
    FROM {{ ref('f_astronomer_task_counts') }}
    GROUP BY 2
...
join_table AS (
    ...
    INNER JOIN {{ source('silver', 'd_data_platform_budget') }} dpb 
    ON duc.metric_date::date BETWEEN dpb.start_date::date AND dpb.end_date::date
        AND ((SPLIT_PART(duc.metric_name,'_',1) = LOWER(dpb.platform) AND duc.metric_name <> 'aws_databricks_costs')
        OR (CONCAT(SPLIT_PART(duc.metric_name,'_',1),' ',SPLIT_PART(duc.metric_name,'_',2)) = LOWER(dpb.platform) AND duc.metric_name = 'aws_databricks_costs'))
