...
,
last_termination_global_event_co AS (
    ...
    FROM f_origination_termination_events_co_logs
    GROUP BY 1,2
)
,
last_termination_all_event_co AS (
...
,
application_stats_and_debug_start_co AS (
   ...
   FROM f_origination_events_co_logs AS oe
   GROUP BY 1
)
,
application_last_event_termination_priority_co AS (
   ...
   FROM      f_origination_events_co              AS oe
   LEFT JOIN last_termination_global_event_co     AS lte_g ON oe.application_id = lte_g.application_id
   LEFT JOIN last_termination_all_event_co        AS lte_a ON oe.application_id = lte_a.application_id
   LEFT JOIN application_stats_and_debug_start_co AS sd    ON oe.application_id = sd.application_id
)
...
FROM application_last_event_termination_priority_co
