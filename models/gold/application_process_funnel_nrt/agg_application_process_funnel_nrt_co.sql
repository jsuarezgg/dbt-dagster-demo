...
WITH data_with_array_struct AS (
    ...
    FROM {{ ref('dm_application_process_funnel_incremental_nrt_co') }}
)
,
...
SELECT 
    ...
    TIMESTAMP(FLOOR(UNIX_TIMESTAMP(ocurred_on_timestamp) / (15 * 60)) * (15 * 60)) AS ocurred_on_date_hour,
    journey_name,
    journey_homolog,
    flag_preapproval,
    channel,
...
e2e as (
  ...
  AND pre.ocurred_on_date_hour = post.ocurred_on_date_hour
