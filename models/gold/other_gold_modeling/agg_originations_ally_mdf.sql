...
with co_originations AS (
    ...
    FROM {{ ref('f_applications_co') }} AS apps
    INNER JOIN {{ ref('f_originations_bnpl_co') }} AS orig 					ON apps.application_id = orig.application_id
    LEFT JOIN {{ ref('f_loan_proposals_co') }} AS lp 						ON orig.loan_id = lp.loan_proposal_id
