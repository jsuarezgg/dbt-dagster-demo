...
--bronze.kustomer_conversations_custom_attributes
...
kus_conversation_custom_attributes AS (
    ...
    FROM {{ ref('kustomer_conversations_custom_attributes') }}
    {%- if is_incremental() %}
    -- DBT INCREMENTAL SENTENCE
    WHERE to_date(reference_date) BETWEEN (to_date("{{ var('start_date') }}"- INTERVAL "{{var('incremental_slack_time_in_days')}}" DAY)) AND to_date("{{ var('end_date') }}")
    AND CAST(reference_date AS TIMESTAMP) BETWEEN (to_timestamp("{{ var('start_date') }}"- INTERVAL "{{var('incremental_slack_time_in_days')}}" DAY)) AND to_timestamp("{{ var('end_date')}}")
    {%- endif -%}
)
,
{%- if is_incremental() %}
