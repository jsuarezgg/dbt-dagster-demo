...
SELECT
    ...
    NULLIF(TRIM(json_tmp.ally.webpage),'') AS ally_webpage,
    ...
    NULLIF(TRIM(json_tmp.ally.additionalInformation.address.lineOne),'') AS ally_address_line_one,
    --NULLIF(TRIM(json_tmp.ally.additionalInformation.address.postalCode),'') AS ally_address_postal_code,
    ...
    json_tmp.ally.additionalInformation.financialInformation.numberCheckDigit AS ally_financial_information_number_check_digit,
    json_tmp.ally.additionalInformation.financialInformation.type AS ally_financial_information_type,
    json_tmp.ally.additionalInformation.legalIdentification.checkDigit AS ally_legal_identification_check_digit,
    NULLIF(TRIM(json_tmp.ally.additionalInformation.legalIdentification.name),'') AS ally_legal_identification_name,
    NULLIF(TRIM(json_tmp.ally.additionalInformation.legalIdentification.number),'') AS ally_legal_identification_number,
    ...
    NULLIF(TRIM(json_tmp.ally.additionalInformation.notificationInformation.email),'') AS ally_notification_information_email,
    --json_tmp.ally.additionalInformation.notificationInformation.name AS ally_notification_information_name,
    NULLIF(TRIM(json_tmp.ally.additionalInformation.notificationInformation.phone),'') AS ally_notification_information_phone
    -- CUSTOM ATTRIBUTES
    -- Fill with your custom attributes
    -- CAST(ocurred_on AS TIMESTAMP) AS allyupdated_co_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'allyupdated_co') }}
