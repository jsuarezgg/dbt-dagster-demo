...
--raw_modeling.loanproposalselectedsantanderco_co
SELECT
    -- MANDATORY FIELDS
    ...
    CAST(1 AS TINYINT) AS chose_loan_proposal,
    CAST(TRUE AS BOOLEAN) AS custom_is_santander_branched
    -- CAST(ocurred_on AS TIMESTAMP) AS loanproposalselectedsantanderco_co_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'loanproposalselectedsantanderco_co') }}
