...
--raw_modeling.pseinvoicedeclinedco_co
SELECT
    -- MANDATORY FIELDS
    ...
    COALESCE(json_tmp.payment.rejection.code, json_tmp.application.fullPayment.pse.rejection.code) AS payment_rejection_code,
    COALESCE(json_tmp.payment.rejection.description, json_tmp.application.fullPayment.pse.rejection.description) AS payment_rejection_description,
    COALESCE(CAST(json_tmp.application.fullPayment.pse.isMarketingOptIn AS BOOLEAN),
             ...
             CAST(json_tmp.payment.payer.isMarketingOptIn AS BOOLEAN),
             CAST(json_tmp.application.fullPayment.pse.payer.isMarketingOptIn AS BOOLEAN)) AS is_marketing_opt_in
    -- CUSTOM ATTRIBUTES
      -- Fill with your custom attributes
    -- CAST(ocurred_on AS TIMESTAMP) AS pseinvoicedeclinedco_co_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'pseinvoicedeclinedco_co') }}
