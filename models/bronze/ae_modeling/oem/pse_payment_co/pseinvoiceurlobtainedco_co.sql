...
--raw_modeling.pseinvoiceurlobtainedco_co
SELECT
    -- MANDATORY FIELDS
    ...
    COALESCE(CAST(json_tmp.payment.isMarketingOptIn AS BOOLEAN),
             CAST(json_tmp.application.fullPayment.pse.isMarketingOptIn AS BOOLEAN),
    ...
    -- CAST(ocurred_on AS TIMESTAMP) AS pseinvoiceurlobtainedco_co_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'pseinvoiceurlobtainedco_co') }}
