...
--raw_modeling.creditcheckrejectedsantanderco_co
SELECT
    -- MANDATORY FIELDS
    ...
    /* coalesce(json_tmp.creditCheck.scoreV2.value,
             json_tmp.creditCheck.score.value) as credit_score_name,*/
    ...
    -- json_tmp.creditCheck.shouldBeBlackListed as should_be_black_listed,
    ...
    -- CAST(ocurred_on AS TIMESTAMP) AS creditcheckrejectedsantanderco_co_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'creditcheckrejectedsantanderco_co') }}
