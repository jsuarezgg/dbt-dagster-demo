...
SELECT
    ...
    -- CAST(ocurred_on AS TIMESTAMP) AS basicidentityvalidatedco_co_at -- To store it as a standalone column, when needed
    CAST(1 AS TINYINT) AS identity_verification_passed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'basicidentityvalidatedco_co') }}
