...
--raw_modeling.preapprovalapplicationcompletedco_co
SELECT
    -- MANDATORY FIELDS
    ...
    json_tmp.client.preapproval.amount AS preapproval_amount,
    json_tmp.client.preapproval.expirationDate AS preapproval_expiration_date,
    -- CUSTOM ATTRIBUTES
      -- Fill with your custom attributes
   'PREAPPROVAL' AS application_type,
    CAST(1 AS TINYINT) AS prospect_application_preapproved,
    CAST(TRUE AS BOOLEAN) AS custom_is_preapproval_completed
    -- CAST(ocurred_on AS TIMESTAMP) AS preapprovalapplicationcompletedco_co_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'preapprovalapplicationcompletedco_co') }}
