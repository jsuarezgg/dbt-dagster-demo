...
--raw_modeling.emailageobtained_br
WITH select_explode AS (
    SELECT
        ...
        EXPLODE(json_tmp.emailAge.socialMedia) AS item
        -- CUSTOM ATTRIBUTES
        -- CAST(ocurred_on AS TIMESTAMP) AS emailageobtained_br_at -- To store it as a standalone column, when needed
    FROM  {{source(  'raw_modeling', 'emailageobtained_br' )}}
...
SELECT
    ...
    application_id,
    client_id,
    'json_tmp.emailAge.socialMedia._ARRAY_' AS array_parent_path,
    ocurred_on,
    ocurred_on_date,
    ingested_at,
    updated_at,
    ...
    -- ITEM INNER FIELDS
    item.link AS emailage_social_media_link,
    item.source AS emailage_social_media_source
