...
WITH select_explode AS (
    ...
    FROM  {{source(  'raw_modeling', 'prospectbureaucontactinfoobtained_br' )}}
...
SELECT
    ...
    -- ITEM INNER FIELDS
    item.addressComplement AS bureau_address_addressComplement,
    item.city AS bureau_address_city,
    item.country AS bureau_address_country,
    item.district AS bureau_address_district,
    TO_TIMESTAMP(item.firstReport) AS bureau_address_firstReport,
    item.homeNumber AS bureau_address_homeNumber,
    item.homeStreet AS bureau_address_homeStreet,
    item.order AS bureau_address_order,
    item.stateAcronym AS bureau_address_stateAcronym,
    item.street AS bureau_address_street,
    item.type AS bureau_address_type,
    item.zipCode AS bureau_address_zipCode
