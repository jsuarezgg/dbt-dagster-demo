...
--raw_modeling.conditionalcreditcheckpassedwithlowerapprovedamountbr_br
WITH select_explode AS (
    SELECT
        ...
        json_tmp.application.downPayment as downpayment,
        json_tmp.creditCheck.downPaymentMethods as downpayment_methods,
        -- CUSTOM ATTRIBUTES
        CAST(true as BOOLEAN) as returning_client,
        EXPLODE(json_tmp.creditCheck.loanProposals) as loan_proposals_event
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'conditionalcreditcheckpassedwithlowerapprovedamountbr_br') }}
...
SELECT 
    ...
    loan_proposals_event.downPaymentAmount as down_payment_amount,
    loan_proposals_event.hasDownPayment as has_downpayment
FROM select_explode