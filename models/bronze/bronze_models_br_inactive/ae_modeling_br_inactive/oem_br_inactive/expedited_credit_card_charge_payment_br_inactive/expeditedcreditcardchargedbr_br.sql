...
--raw_modeling.expeditedcreditcardchargedbr_br
SELECT
    -- MANDATORY FIELDS
    ...
    coalesce(json_tmp.application.requestedAmount,
             json_tmp.application.requestedAmountWithoutDiscount) as approved_amount,
    --json_tmp.application.id as loan_id
    CAST(json_tmp.ocurredOn AS TIMESTAMP) AS origination_date,
    json_tmp.metadata.context.storeId as store_slug,
    json_tmp.metadata.context.storeUserName as store_user_id
    -- CUSTOM ATTRIBUTES
      -- Fill with your custom attributes
    -- CAST(ocurred_on AS TIMESTAMP) AS expeditedcreditcardchargedbr_br_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'expeditedcreditcardchargedbr_br') }}
