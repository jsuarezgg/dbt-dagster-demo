...
--raw_modeling.identitywadiscardedbyrisk_br
SELECT
    -- MANDATORY FIELDS
    ...
    CAST(ocurred_on AS TIMESTAMP) AS identitywadiscardedbyrisk_co_at, -- To store it as a standalone column, when needed
    CAST(ocurred_on AS TIMESTAMP) AS identitywadiscardedbyrisk_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'identitywadiscardedbyrisk_br') }}
