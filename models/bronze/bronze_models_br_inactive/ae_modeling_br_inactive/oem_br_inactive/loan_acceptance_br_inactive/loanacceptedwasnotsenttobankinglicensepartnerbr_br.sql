...
--raw_modeling.loanacceptedwasnotsenttobankinglicensepartnerbr_br
SELECT
    -- MANDATORY FIELDS
    ...
    json_tmp.loan.hasDownPayment AS has_downpayment,
    -- CUSTOM ATTRIBUTES
    {{ dbt_utils.surrogate_key(['event_id', 'json_tmp.loan.id']) }} AS surrogate_key
    -- CAST(ocurred_on AS TIMESTAMP) AS loanacceptedwasnotsenttobankinglicensepartnerbr_br_at -- To store it as a standalone column, when needed
-- DBT SOURCE REFERENCE
from {{ source('raw_modeling', 'loanacceptedwasnotsenttobankinglicensepartnerbr_br') }}
