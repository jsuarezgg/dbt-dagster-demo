...
d_employees_id_co AS (
    ...
    SELECT * FROM silver.d_employees_id_co
...
f_syc_condonations_co AS (
    ...
    SELECT * FROM silver.f_syc_condonations_co

),

d_snc_client_loan_calculations_co AS (

    SELECT * FROM silver.d_snc_client_loan_calculations_co

),

dm_originations_base AS (

    SELECT * FROM gold.dm_originations

),

d_syc_usury_rates_co AS (

    SELECT * FROM silver.d_syc_usury_rates_co

),

loan_ownership_co AS (

    SELECT * FROM gold.loan_ownership_co

),

pap_exclusion_loan_sale AS (

    SELECT * FROM risk.pap_exclusion_loan_sale
...
reason_to_filter_loans AS (
    ...
    SELECT * FROM risk.reason_to_filter_loans
...
dm_grande_lbl_eligibility_criteria AS (
    ...
    FROM ds.dm_grande_lbl_eligibility_criteria

),

f_refinance_loans_co AS (

    SELECT * FROM silver.f_refinance_loans_co

),

f_kyc_bureau_income_estimator_co AS (

    SELECT application_id, income_estimatedIncome FROM silver.f_kyc_bureau_income_estimator_co

),

f_kyc_bureau_personal_info_co AS (

    SELECT application_id, personId_ageRange FROM silver.f_kyc_bureau_personal_info_co

),

f_underwriting_fraud_stage_co AS (

    SELECT application_id, credit_check_income_provider, tdsr FROM silver.f_underwriting_fraud_stage_co
...
FROM gold.dm_loan_cancellations_co
