...
FROM  silver.f_hubspot_deals_details_log h
WHERE 1=1
...
FROM silver.d_salesforce_opportunities
WHERE 1=1
AND opportunity_slug IS NOT NULL AND opportunity_record_type_name != 'Addishop' AND opportunity_is_deleted = False
...
FROM silver.d_ally_management_allies_br al 
LEFT JOIN hubspot h
...
base_co AS (
  ...
  "CO" as country_code,
	...
	CASE WHEN COALESCE(s.opportunity_ally_cluster,h.ally_cluster)='Corporate' THEN 'KA'
...
FROM silver.d_ally_management_allies_co al 
