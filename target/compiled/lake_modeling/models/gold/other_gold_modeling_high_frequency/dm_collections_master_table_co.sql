...
FROM gold.dm_loan_status_co ls
LEFT JOIN gold.risk_master_table_co rmt ON rmt.loan_id=ls.loan_id
...
FROM silver.f_syc_refinancing_instructions_co sri
...
FROM silver.f_talkdesk_calls_report_co
...
AND disposition_code = 'Contactado' and start_at is not null
GROUP BY 1)
...
FROM (
SELECT resourceid,
reverse(split(customAttributeValue,'[.]'))[0] LastResolTreeValue,
referencedate,
max(referenceDate) over (PARTITION BY resourceid) max_date
FROM cur.kus_custom_attributes
WHERE customAttributeName LIKE '%resolutionCollectionsTree%')
...
datediff(current_date,createdat) DaysDiffLastResol
FROM cur.kus_conversations con
LEFT JOIN query5 q5
ON con.conversationid=q5.resourceid
INNER JOIN cur.crm_clients crm
ON con.customerid=crm.kustomer_id)
...
FROM silver.f_client_payment_promises_co cpp
LEFT JOIN silver.f_fincore_payments_co fp
...
max(state) state,
max(expected_amount) expected_amount,
active_promise,
max(end_date) end_date,
max(start_date::date) start_date,
max(paid_amount) paid_amount
FROM query10
WHERE end_date::date=max_end_date::date
GROUP BY client_id, active_promise 
),
...
,CASE
    ...
    END
    AS idv_bypassed_or
FROM gold.fraud_master_table_co fmt
LEFT JOIN gold.risk_master_table_co mt
    ON fmt.application_id = mt.application_id
LEFT JOIN silver.f_fincore_loans_co fl
...
LEFT JOIN cur.client_exclusion_list cel
