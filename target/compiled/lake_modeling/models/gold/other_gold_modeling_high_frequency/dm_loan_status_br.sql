...
WITH syc_loan_status AS (
    ...
    SELECT * FROM silver.d_syc_loan_status_br

),
snc_payments_report AS (

    SELECT * FROM silver.f_snc_payments_report_br

),
syc_originations AS (

    SELECT * FROM silver.f_syc_originations_br

),
client_loans AS (

    SELECT * FROM silver.f_fincore_loans_br

),
dynamic_funnel_loan_proposals AS (

    SELECT * FROM silver.f_loan_proposals_br

),
originations AS (

    SELECT * FROM silver.f_originations_bnpl_br
...
allies AS (
    ...
    SELECT * FROM silver.d_ally_management_stores_allies_br
