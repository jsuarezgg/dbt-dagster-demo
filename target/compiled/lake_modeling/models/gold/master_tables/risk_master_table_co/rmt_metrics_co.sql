...
with initial_table as (
  select orig.loan_id,
         from_utc_timestamp(first_payment_date, 'America/Bogota') as first_payment_date
  from silver.f_applications_co apps
  inner join silver.f_originations_bnpl_co orig
    on apps.application_id = orig.application_id
  left join gold.dm_loan_status_co ls
...
, dpd_w_dates as (
  ...
  left join gold.dm_daily_loan_status_co as dls
...
,condonation_by_mob as (
  ...
  from gold.rmt_condonation_date_co cd
