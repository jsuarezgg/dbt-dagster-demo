...
left join gold.dm_loan_status_br ls on ls.loan_id = orig.loan_id
left join gold.fmt_bureau_check_br bc on a.application_id = bc.application_id
left join gold.fmt_client_info_br ci on a.client_id = ci.prospect_id
left join gold.fmt_repeat_email_br re on a.application_id = re.application_id
left join gold.fmt_repeat_cellphone_br rc on a.application_id = rc.application_id
left join gold.fmt_repeat_ip_address_br ri on a.application_id = ri.application_id
left join gold.fmt_changed_app_br ca on a.application_id = ca.application_id
left join gold.fmt_repeated_address_br ra on a.application_id = ra.application_id
left join gold.fmt_clean_phone_email_match_br cpem on a.application_id = cpem.application_id
left join gold.fmt_bt_walk_down_br btwd on a.application_id = btwd.application_id
left join gold.fmt_prev_app_information_br pa on a.application_id = pa.application_id
left join gold.fmt_emailage_br em on a.application_id = em.application_id
left join gold.fmt_iovation_br io on a.application_id = io.application_id
left join gold.fmt_neuroid_br nid on a.application_id = nid.application_id
left join gold.fmt_kyc_all_info_br kyc on a.application_id = kyc.application_id
