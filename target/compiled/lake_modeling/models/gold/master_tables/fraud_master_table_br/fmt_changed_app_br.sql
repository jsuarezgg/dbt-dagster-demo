...
with loan_data as (
  ...
  loan_id,
  approved_amount,
  from_utc_timestamp(origination_date, "America/Sao_Paulo") AS origination_date,
  state,
  first_value(from_utc_timestamp(origination_date, "America/Sao_Paulo")) over (partition by client_id order by origination_date) as first_origination_date,
  first_value(from_utc_timestamp(origination_date, "America/Sao_Paulo")) over (partition by client_id order by origination_date desc) as last_origination_date,
  lag(from_utc_timestamp(origination_date, "America/Sao_Paulo")) over (partition by client_id order by origination_date) as prev_origination_date,
  row_number() over (partition by client_id order by origination_date) as rn
  from gold.dm_loan_status_br
),
preap_apps as (
  select *,
  prospect_id as client_id
  from gold.rmt_application_preapproval_br
  where rn = 1
),
vertical AS (
	SELECT
...
app_count as (
  ...
  orig.loan_id,
  a.application_id,
  from_utc_timestamp(a.application_date, "America/Sao_Paulo") AS application_date,
  a.ally_slug as ally_name,
  a.requested_amount,
  v.vertical,
  ld.first_origination_date,
  case when from_utc_timestamp(a.application_date, "America/Sao_Paulo") < ld.first_origination_date or ld.first_origination_date is null then 'PROSPECT' else 'CLIENT' end as client_type
  from silver.f_applications_br a
  left join silver.f_originations_bnpl_br orig
