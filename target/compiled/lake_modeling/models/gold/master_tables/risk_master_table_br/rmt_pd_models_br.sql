...
from bronze.prospect_br_credit_evaluation_results_br
...
select application_id,
       ...
       evaluation_result:['segment']['policy_name'] as credit_policy_name,
       row_number() over(partition by application_id order by created_at desc) as rn
from silver.f_pre_approvals_br_credit_evaluation_results_br
...
from silver.f_prospect_br_credit_evaluation_results_br
...
select context_id as application_id,
       ...
       row_number() over(partition by context_id order by created_at desc) as rn
from silver.f_risk_service_decision_unit_logs_br dul
...
select application_id,
       ...
       row_number() over(partition by application_id order by created_at desc) as rn
from silver.f_prospect_br_prospect_evaluation_results_br
