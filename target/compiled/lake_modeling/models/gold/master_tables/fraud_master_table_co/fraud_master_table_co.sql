...
left join silver.f_underwriting_fraud_stage_co udw on a.application_id = udw.application_id
left join silver.f_originations_bnpl_co orig on a.application_id = orig.application_id
left join gold.dm_loan_status_co ls on orig.loan_id = ls.loan_id
left join gold.fmt_client_info_co ci on a.client_id = ci.prospect_id 
left join gold.fmt_emailage_co em on a.application_id = em.application_id
left join gold.fmt_neuroid_co nid on a.application_id = nid.application_id
left join gold.fmt_telesign_co ts on a.application_id = ts.application_id
left join gold.fmt_iovation_co io on a.application_id = io.application_id
left join gold.fmt_kyc_all_info_co kyc on a.application_id = kyc.application_id
left join gold.fmt_bureau_check_co bc on a.application_id = bc.application_id
left join gold.fmt_repeat_email_co re on a.application_id = re.application_id
left join gold.fmt_repeat_cellphone_co rc on a.application_id = rc.application_id
left join gold.fmt_repeat_ip_address_co ri on a.application_id = ri.application_id
left join gold.fmt_changed_app_co ca on a.application_id = ca.application_id 
left join gold.fmt_repeated_address_co ra on a.application_id = ra.application_id
left join gold.fmt_clean_phone_email_match_co cpem on a.application_id = cpem.application_id
left join gold.fmt_bt_walk_down_co btwd on a.application_id = btwd.application_id
left join gold.fmt_prev_app_information_co pa on a.application_id = pa.application_id
