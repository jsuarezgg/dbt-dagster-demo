...
addicupo_final as (
  ...
  FROM
    ...
    from silver.f_syc_clients_addicupo_history_co
    )
  WHERE rn = 1
)
,
  ...
  select  *  from
    (
      ...
      select
        ...
        client_id,
        proba_pd_rc
      from ds.back_test_prediction_202202_202211_rc_co_v2
      )
      union all
      (
      select
        ...
        client_id,
        proba_pd_rc
      from risk.predictions_returning_client_model_cl_level
...
SELECT
  ...
  from gold.risk_master_table_co AS a
...
left join silver.d_syc_clients_co                      AS c on a.prospect_id = c.client_id
left join risk_lvl                                           AS d on a.prospect_id = d.client_id
left join addicupo_history                                   AS e on a.prospect_id = e.client_id and (a.d_vintage between e.addicupo_last_update and e.addicupo_last_update_v2)
