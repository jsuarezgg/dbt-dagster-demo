...
WHERE event_type IN ('HOME_STORE_TAPPED','SHOP_STORE_TAPPED') AND user_id IS NOT NULL AND ally_name IS NOT NULL AND to_date(event_time) >= '2022-06-03'
)
,
frontend_events AS (
...
interest_originations_br as (
    ...
    FROM silver.f_applications_br as application_br
...
originations_co as (
    SELECT
        ...
        o_bnpl_co.loan_id
    FROM silver.f_originations_bnpl_co as o_bnpl_co
...
interest_originations_co as (
    ...
    FROM silver.f_applications_co AS io_co
...
loans_meeting_criteria_7days(
    SELECT io.loan_id,
           ...
           COLLECT_SET(named_struct('source',ost.my_source,'dt_utc',ost.client_event_time,'screen',screen_name,'component',component_name)) AS sources_7days,
...
loans_meeting_criteria_2days(
     SELECT io.loan_id,
           ...
           COLLECT_SET(named_struct('source',ost.my_source,'dt_utc',ost.client_event_time,'screen',screen_name,'component',component_name)) AS sources_2days,
