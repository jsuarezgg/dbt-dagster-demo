...
with co_originations AS (
    ...
    FROM silver.f_applications_co AS apps
    INNER JOIN silver.f_originations_bnpl_co AS orig 					ON apps.application_id = orig.application_id
    LEFT JOIN silver.f_loan_proposals_co AS lp 						ON orig.loan_id = lp.loan_proposal_id
...
br_originations AS (
    ...
    FROM silver.f_applications_br AS apps
    INNER JOIN silver.f_originations_bnpl_br AS orig 					ON apps.application_id = orig.application_id
    LEFT JOIN silver.f_loan_proposals_br AS lp 						ON orig.loan_id = lp.loan_proposal_id
