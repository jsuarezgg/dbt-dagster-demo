...
WITH
shoppingintentregistered_co AS ( 
    SELECT *
    FROM bronze.shoppingintentregistered_co
...
, union_bronze AS (
    ...
    event_id
    FROM shoppingintentregistered_co
...
, union_all_events AS (
    ...
    event_id
    FROM silver.f_marketplace_shopping_intents_co  
    WHERE 
    silver.f_marketplace_shopping_intents_co.shopping_intent_id IN (SELECT DISTINCT shopping_intent_id FROM union_bronze)      
...
/* DEBUGGING SECTION
is_incremental: True
this: silver.f_marketplace_shopping_intents_co
country: co
silver_table_name: f_marketplace_shopping_intents_co
...
events_dict: {'ShoppingIntentRegistered': {'stage': 'shoppingintentregistered_co', 'direct_attributes': ['ally_slug', 'shopping_intent_id', 'client_id', 'campaign_id', 'channel', 'ocurred_on', 'device_id'], 'custom_attributes': {}}}
