...
WITH
transactionattributable_br AS ( 
    SELECT *
    FROM bronze.transactionattributable_br
...
, union_bronze AS (
    ...
    event_id
    FROM transactionattributable_br
...
, union_all_events AS (
    ...
    event_id
    FROM silver.f_marketplace_transaction_attributable_br  
    WHERE 
    silver.f_marketplace_transaction_attributable_br.application_id IN (SELECT DISTINCT application_id FROM union_bronze)      
...
/* DEBUGGING SECTION
is_incremental: True
this: silver.f_marketplace_transaction_attributable_br
country: br
silver_table_name: f_marketplace_transaction_attributable_br
table_pk_fields: ['application_id']
table_pk_amount: 1
fields_direct: ['application_id', 'client_id', 'ocurred_on', 'shopping_intent_id', 'device_id']
mandatory_fields: ['event_name', 'event_id']
events_dict: {'TransactionAttributable': {'stage': 'transaction_attributable_br', 'direct_attributes': ['application_id', 'client_id', 'shopping_intent_id', 'ocurred_on', 'device_id'], 'custom_attributes': {}}}
