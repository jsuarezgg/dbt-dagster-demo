...
, union_bronze AS (
    SELECT 
        additional_attributes,ocurred_on,restricted_entity_created_at,restricted_entity_journey,restricted_entity_reason,restricted_entity_reference,restricted_entity_source,restricted_entity_status,restricted_entity_type,restricted_entity_value,surrogate_key,event_name as last_event_name_processed,
    ...
    UNION ALL
    SELECT 
        NULL as additional_attributes,ocurred_on,restricted_entity_created_at,restricted_entity_journey,restricted_entity_reason,restricted_entity_reference,restricted_entity_source,restricted_entity_status,restricted_entity_type,restricted_entity_value,surrogate_key,event_name as last_event_name_processed,
    event_name,event_id as last_event_id_processed,
    event_id
...
, union_all_events AS (
    ...
    UNION ALL
    SELECT 
    additional_attributes,last_event_ocurred_on_processed as ocurred_on,restricted_entity_created_at,restricted_entity_journey,restricted_entity_reason,restricted_entity_reference,restricted_entity_source,restricted_entity_status,restricted_entity_type,restricted_entity_value,surrogate_key,last_event_name_processed,
    event_name,last_event_id_processed,
    event_id
    FROM silver.d_restricted_entities_co  
    WHERE 
    silver.d_restricted_entities_co.surrogate_key IN (SELECT DISTINCT surrogate_key FROM union_bronze)      
...
-- SECTION 3 -> merge events by key and keep last not null data in each field
  ...
  select
    surrogate_key,
    element_at(array_sort(array_agg(CASE WHEN additional_attributes is not null then struct(ocurred_on, additional_attributes) else NULL end), (left, right) -> case when left.ocurred_on < right.ocurred_on then 1 when left.ocurred_on > right.ocurred_on then -1 when left.ocurred_on == right.ocurred_on then 0 end), 1).additional_attributes as additional_attributes,
...
/* DEBUGGING SECTION
is_incremental: True
this: silver.d_restricted_entities_co
country: co
silver_table_name: d_restricted_entities_co
table_pk_fields: ['surrogate_key']
table_pk_amount: 1
fields_direct: ['additional_attributes', 'ocurred_on', 'restricted_entity_created_at', 'restricted_entity_journey', 'restricted_entity_reason', 'restricted_entity_reference', 'restricted_entity_source', 'restricted_entity_status', 'restricted_entity_type', 'restricted_entity_value', 'surrogate_key']
mandatory_fields: ['event_name', 'event_id']
events_dict: {'restrictedentitysuspected': {'direct_attributes': ['surrogate_key', 'restricted_entity_created_at', 'restricted_entity_journey', 'restricted_entity_reason', 'restricted_entity_reference', 'restricted_entity_source', 'restricted_entity_status', 'restricted_entity_type', 'restricted_entity_value', 'additional_attributes', 'ocurred_on'], 'custom_attributes': {}}, 'restrictedentityblocked': {'direct_attributes': ['surrogate_key', 'restricted_entity_created_at', 'restricted_entity_journey', 'restricted_entity_reason', 'restricted_entity_reference', 'restricted_entity_source', 'restricted_entity_status', 'restricted_entity_type', 'restricted_entity_value', 'additional_attributes', 'ocurred_on'], 'custom_attributes': {}}, 'restrictedentityactivated': {'direct_attributes': ['surrogate_key', 'restricted_entity_created_at', 'restricted_entity_journey', 'restricted_entity_reason', 'restricted_entity_reference', 'restricted_entity_source', 'restricted_entity_status', 'restricted_entity_type', 'restricted_entity_value', 'ocurred_on'], 'custom_attributes': {}}}
