...
WITH
loancancellationorderprocessed_co AS ( 
    SELECT *
    FROM bronze.loancancellationorderprocessed_co
...
, union_bronze AS (
    SELECT 
        ally_slug,client_id,custom_event_domain,custom_loan_cancellation_status,loan_approved_amount,NULL as loan_cancellation_amount,NULL as loan_cancellation_annulment_reason,loan_cancellation_id,loan_cancellation_order_date,loan_cancellation_reason,NULL as loan_cancellation_type,loan_id,loan_origination_date,ocurred_on,NULL as store_user_name,user_id,NULL as user_role,event_name as last_event_name_processed,
    event_name,event_id as last_event_id_processed,
    event_id
    FROM loancancellationorderprocessed_co
    UNION ALL
    SELECT 
...
, union_all_events AS (
    ...
    event_id
    FROM silver.f_loan_cancellations_v2_co  
    WHERE 
    silver.f_loan_cancellations_v2_co.loan_id IN (SELECT DISTINCT loan_id FROM union_bronze)      
...
/* DEBUGGING SECTION
is_incremental: True
this: silver.f_loan_cancellations_v2_co
country: co
silver_table_name: f_loan_cancellations_v2_co
...
events_dict: {'loancancellationorderprocessed': {'direct_attributes': ['loan_id', 'ally_slug', 'client_id', 'user_id', 'loan_cancellation_id', 'loan_cancellation_reason', 'loan_cancellation_order_date', 'loan_approved_amount', 'loan_origination_date', 'custom_event_domain', 'custom_loan_cancellation_status', 'ocurred_on'], 'custom_attributes': {}}, 'loancancellationorderprocessedv2': {'direct_attributes': ['loan_id', 'ally_slug', 'client_id', 'user_id', 'loan_cancellation_id', 'loan_cancellation_amount', 'loan_cancellation_type', 'loan_cancellation_reason', 'loan_cancellation_order_date', 'loan_approved_amount', 'loan_origination_date', 'user_role', 'store_user_name', 'custom_event_domain', 'custom_loan_cancellation_status', 'ocurred_on'], 'custom_attributes': {}}, 'loancancellationorderannulledv2': {'direct_attributes': ['loan_id', 'client_id', 'user_id', 'loan_cancellation_annulment_reason', 'loan_cancellation_id', 'custom_event_domain', 'custom_loan_cancellation_status', 'ocurred_on'], 'custom_attributes': {}}}
events_keys: ['loancancellationorderprocessed', 'loancancellationorderprocessedv2', 'loancancellationorderannulledv2']
