...
WITH
iovationobtained_unnested_by_result_rule_br AS ( 
    SELECT *
    FROM bronze.iovationobtained_unnested_by_result_rule_br
    WHERE ocurred_on_date BETWEEN (to_date('2022-01-01'- INTERVAL "10" HOUR)) AND to_date('2022-01-30') AND
        ocurred_on BETWEEN (to_timestamp('2022-01-01'- INTERVAL "10" HOUR)) AND to_timestamp('2022-01-30') 
)
,iovationobtained_v2_unnested_by_result_rule_br AS ( 
    SELECT *
    FROM bronze.iovationobtained_v2_unnested_by_result_rule_br
    WHERE ocurred_on_date BETWEEN (to_date('2022-01-01'- INTERVAL "10" HOUR)) AND to_date('2022-01-30') AND
        ocurred_on BETWEEN (to_timestamp('2022-01-01'- INTERVAL "10" HOUR)) AND to_timestamp('2022-01-30') 
)
...
, union_bronze AS (
    ...
    event_name,
    event_id
    FROM iovationobtained_unnested_by_result_rule_br
    UNION ALL
    SELECT 
        application_id,array_parent_path,client_id,custom_kyc_event_version,iovation_rule_result_reason,iovation_rule_result_score,iovation_rule_result_type,item_pseudo_idx,ocurred_on,surrogate_key,
    event_name,
    event_id
    FROM iovationobtained_v2_unnested_by_result_rule_br
...
/* DEBUGGING SECTION
is_incremental: True
this: silver.f_kyc_iovation_v1v2_result_rules_br_logs
country: br
silver_table_name: f_kyc_iovation_v1v2_result_rules_br_logs
