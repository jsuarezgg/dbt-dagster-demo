...
WITH
clientloansstatusupdatedv2_unnested_by_loan_id_co AS ( 
    SELECT *
...
,loanguaranteepublishedtofund_co AS ( 
    SELECT *
    FROM bronze.loanguaranteepublishedtofund_co
    WHERE ocurred_on_date BETWEEN (to_date('2022-01-01'- INTERVAL "10" HOUR)) AND to_date('2022-01-30') AND
        ocurred_on BETWEEN (to_timestamp('2022-01-01'- INTERVAL "10" HOUR)) AND to_timestamp('2022-01-30') 
)
...
, union_all_events AS (
    SELECT 
        applicable_rate,application_id,approved_amount,client_id,current_installment_amount,custom_fincore_included,custom_to_fga,days_past_due,delinquency_balance,down_payment_amount,effective_annual_rate,first_payment_date,full_payment,guarantee_overdue,initial_installment_amount,interest_on_overdue_principal,interest_overdue,is_fully_paid,late_fees,loan_id,loan_type,min_payment,months_on_book,last_event_ocurred_on_processed as ocurred_on,origination_date,paid_installments,principal_overdue,state,term,total_guarantee_paid,total_interest_paid,total_late_fees_condoned,total_late_fees_paid,total_payment_applied,total_principal_paid,unpaid_guarantee,unpaid_principal,last_event_name_processed,
    event_name,last_event_id_processed,
    event_id
    FROM silver.f_fincore_loans_co
    WHERE silver.f_fincore_loans_co.ocurred_on_date BETWEEN (to_date('2022-01-01'- INTERVAL "10" HOUR)) AND to_date('2022-01-30')
    UNION ALL
    SELECT 
        applicable_rate,NULL as application_id,approved_amount,client_id,current_installment_amount,custom_fincore_included,NULL as custom_to_fga,days_past_due,delinquency_balance,NULL as down_payment_amount,effective_annual_rate,first_payment_date,full_payment,guarantee_overdue,initial_installment_amount,interest_on_overdue_principal,interest_overdue,is_fully_paid,late_fees,loan_id,NULL as loan_type,min_payment,months_on_book,ocurred_on,origination_date,paid_installments,principal_overdue,state,term,total_guarantee_paid,total_interest_paid,total_late_fees_condoned,total_late_fees_paid,total_payment_applied,total_principal_paid,unpaid_guarantee,unpaid_principal,event_name as last_event_name_processed,
    event_name,event_id as last_event_id_processed,
    event_id
    FROM clientloansstatusupdatedv2_unnested_by_loan_id_co
    UNION ALL
    SELECT 
        NULL as applicable_rate,application_id,approved_amount,client_id,NULL as current_installment_amount,NULL as custom_fincore_included,NULL as custom_to_fga,NULL as days_past_due,NULL as delinquency_balance,NULL as down_payment_amount,effective_annual_rate,first_payment_date,NULL as full_payment,NULL as guarantee_overdue,NULL as initial_installment_amount,NULL as interest_on_overdue_principal,NULL as interest_overdue,NULL as is_fully_paid,NULL as late_fees,loan_id,NULL as loan_type,NULL as min_payment,NULL as months_on_book,ocurred_on,origination_date,NULL as paid_installments,NULL as principal_overdue,NULL as state,term,NULL as total_guarantee_paid,NULL as total_interest_paid,NULL as total_late_fees_condoned,NULL as total_late_fees_paid,NULL as total_payment_applied,NULL as total_principal_paid,NULL as unpaid_guarantee,NULL as unpaid_principal,event_name as last_event_name_processed,
    event_name,event_id as last_event_id_processed,
    event_id
    ...
    UNION ALL
    SELECT 
        NULL as applicable_rate,application_id,approved_amount,client_id,NULL as current_installment_amount,NULL as custom_fincore_included,NULL as custom_to_fga,NULL as days_past_due,NULL as delinquency_balance,down_payment_amount,effective_annual_rate,first_payment_date,NULL as full_payment,NULL as guarantee_overdue,NULL as initial_installment_amount,NULL as interest_on_overdue_principal,NULL as interest_overdue,NULL as is_fully_paid,NULL as late_fees,loan_id,loan_type,NULL as min_payment,NULL as months_on_book,ocurred_on,origination_date,NULL as paid_installments,NULL as principal_overdue,NULL as state,term,NULL as total_guarantee_paid,NULL as total_interest_paid,NULL as total_late_fees_condoned,NULL as total_late_fees_paid,NULL as total_payment_applied,NULL as total_principal_paid,NULL as unpaid_guarantee,NULL as unpaid_principal,event_name as last_event_name_processed,
    event_name,event_id as last_event_id_processed,
    event_id
    FROM loanacceptedco_co
    UNION ALL
    SELECT 
        NULL as applicable_rate,NULL as application_id,NULL as approved_amount,NULL as client_id,NULL as current_installment_amount,NULL as custom_fincore_included,custom_to_fga,NULL as days_past_due,NULL as delinquency_balance,NULL as down_payment_amount,NULL as effective_annual_rate,NULL as first_payment_date,NULL as full_payment,NULL as guarantee_overdue,NULL as initial_installment_amount,NULL as interest_on_overdue_principal,NULL as interest_overdue,NULL as is_fully_paid,NULL as late_fees,loan_id,NULL as loan_type,NULL as min_payment,NULL as months_on_book,ocurred_on,NULL as origination_date,NULL as paid_installments,NULL as principal_overdue,NULL as state,NULL as term,NULL as total_guarantee_paid,NULL as total_interest_paid,NULL as total_late_fees_condoned,NULL as total_late_fees_paid,NULL as total_payment_applied,NULL as total_principal_paid,NULL as unpaid_guarantee,NULL as unpaid_principal,event_name as last_event_name_processed,
    event_name,event_id as last_event_id_processed,
    event_id
    FROM loanguaranteepublishedtofund_co
...
, grouped_events AS (
  select
    ...
    element_at(array_sort(array_agg(CASE WHEN custom_to_fga is not null then struct(ocurred_on, custom_to_fga) else NULL end), (left, right) -> case when left.ocurred_on < right.ocurred_on then 1 when left.ocurred_on > right.ocurred_on then -1 when left.ocurred_on == right.ocurred_on then 0 end), 1).custom_to_fga as custom_to_fga,
    ...
    element_at(array_sort(array_agg(CASE WHEN total_guarantee_paid is not null then struct(ocurred_on, total_guarantee_paid) else NULL end), (left, right) -> case when left.ocurred_on < right.ocurred_on then 1 when left.ocurred_on > right.ocurred_on then -1 when left.ocurred_on == right.ocurred_on then 0 end), 1).total_guarantee_paid as total_guarantee_paid,
...
/* DEBUGGING SECTION
is_incremental: True
this: silver.f_fincore_loans_co
country: co
silver_table_name: f_fincore_loans_co
table_pk_fields: ['loan_id']
table_pk_amount: 1
fields_direct: ['applicable_rate', 'application_id', 'approved_amount', 'client_id', 'current_installment_amount', 'custom_fincore_included', 'custom_to_fga', 'days_past_due', 'delinquency_balance', 'down_payment_amount', 'effective_annual_rate', 'first_payment_date', 'full_payment', 'guarantee_overdue', 'initial_installment_amount', 'interest_on_overdue_principal', 'interest_overdue', 'is_fully_paid', 'late_fees', 'loan_id', 'loan_type', 'min_payment', 'months_on_book', 'ocurred_on', 'origination_date', 'paid_installments', 'principal_overdue', 'state', 'term', 'total_guarantee_paid', 'total_interest_paid', 'total_late_fees_condoned', 'total_late_fees_paid', 'total_payment_applied', 'total_principal_paid', 'unpaid_guarantee', 'unpaid_principal']
mandatory_fields: ['event_name', 'event_id']
events_dict: {'clientloansstatusupdatedv2_unnested_by_loan_id': {'stage': 'legacy_or_non_origination', 'direct_attributes': ['loan_id', 'client_id', 'origination_date', 'term', 'days_past_due', 'paid_installments', 'effective_annual_rate', 'approved_amount', 'total_principal_paid', 'min_payment', 'full_payment', 'delinquency_balance', 'principal_overdue', 'interest_overdue', 'guarantee_overdue', 'unpaid_principal', 'interest_on_overdue_principal', 'initial_installment_amount', 'current_installment_amount', 'unpaid_guarantee', 'total_payment_applied', 'state', 'first_payment_date', 'months_on_book', 'applicable_rate', 'total_interest_paid', 'is_fully_paid', 'late_fees', 'total_late_fees_paid', 'total_late_fees_condoned', 'total_guarantee_paid', 'custom_fincore_included', 'ocurred_on'], 'custom_attributes': {}}, 'ProspectUpgradedToClient': {'stage': 'legacy_or_non_origination', 'direct_attributes': ['loan_id', 'client_id', 'application_id', 'origination_date', 'term', 'effective_annual_rate', 'approved_amount', 'first_payment_date', 'ocurred_on'], 'custom_attributes': {}}, 'ClientLoanAccepted': {'stage': 'legacy_or_non_origination', 'direct_attributes': ['loan_id', 'client_id', 'application_id', 'origination_date', 'term', 'effective_annual_rate', 'approved_amount', 'first_payment_date', 'ocurred_on'], 'custom_attributes': {}}, 'LoanAcceptedCO': {'stage': 'loan_acceptance_co', 'direct_attributes': ['loan_id', 'client_id', 'application_id', 'origination_date', 'term', 'loan_type', 'effective_annual_rate', 'approved_amount', 'first_payment_date', 'down_payment_amount', 'ocurred_on'], 'custom_attributes': {}}, 'LoanGuaranteePublishedToFund': {'stage': 'legacy_or_non_origination', 'direct_attributes': ['loan_id', 'custom_to_fga', 'ocurred_on'], 'custom_attributes': {}}}
events_keys: ['clientloansstatusupdatedv2_unnested_by_loan_id', 'ProspectUpgradedToClient', 'ClientLoanAccepted', 'LoanAcceptedCO', 'LoanGuaranteePublishedToFund']
