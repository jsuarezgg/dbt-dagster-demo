...
, union_all_events AS (
    ...
    UNION ALL
    SELECT 
    annulment_reason,client_id,is_annulled,last_event_ocurred_on_processed as ocurred_on,paid_amount,payment_date,payment_id,payment_method,last_event_name_processed,
    event_name,last_event_id_processed,
    event_id
    FROM silver.f_fincore_payments_br  
    WHERE 
    silver.f_fincore_payments_br.payment_id IN (SELECT DISTINCT payment_id FROM union_bronze)      
...
/* DEBUGGING SECTION
is_incremental: True
this: silver.f_fincore_payments_br
country: br
silver_table_name: f_fincore_payments_br
