...
, union_bronze AS (
    SELECT 
        ally_slug,application_id,approved_amount,client_id,effective_annual_rate,lbl,loan_id,loan_type,ocurred_on,origination_date,store_slug,store_user_id,term,event_name as last_event_name_processed,
    ...
    UNION ALL
    SELECT 
        ally_slug,application_id,approved_amount,client_id,effective_annual_rate,lbl,loan_id,NULL as loan_type,ocurred_on,origination_date,store_slug,store_user_id,term,event_name as last_event_name_processed,
    event_name,event_id as last_event_id_processed,
    event_id
...
, union_all_events AS (
    ...
    UNION ALL
    SELECT 
    ally_slug,application_id,approved_amount,client_id,effective_annual_rate,lbl,loan_id,loan_type,last_event_ocurred_on_processed as ocurred_on,origination_date,store_slug,store_user_id,term,last_event_name_processed,
    event_name,last_event_id_processed,
    event_id
    FROM silver.f_originations_bnpl_br  
    WHERE 
    silver.f_originations_bnpl_br.application_id IN (SELECT DISTINCT application_id FROM union_bronze)      
...
/* DEBUGGING SECTION
is_incremental: True
this: silver.f_originations_bnpl_br
country: br
silver_table_name: f_originations_bnpl_br
table_pk_fields: ['application_id']
table_pk_amount: 1
fields_direct: ['ally_slug', 'application_id', 'approved_amount', 'client_id', 'effective_annual_rate', 'lbl', 'loan_id', 'loan_type', 'ocurred_on', 'origination_date', 'store_slug', 'store_user_id', 'term']
mandatory_fields: ['event_name', 'event_id']
events_dict: {'LoanAcceptedByBankingLicensePartnerBR': {'stage': 'loan_acceptance_br', 'direct_attributes': ['application_id', 'client_id', 'loan_id', 'origination_date', 'approved_amount', 'ally_slug', 'store_slug', 'store_user_id', 'term', 'loan_type', 'effective_annual_rate', 'lbl', 'ocurred_on'], 'custom_attributes': {}}, 'LoanAcceptedWasNotSentToBankingLicensePartnerBR': {'stage': 'loan_acceptance_br', 'direct_attributes': ['application_id', 'client_id', 'loan_id', 'origination_date', 'approved_amount', 'loan_type', 'ally_slug', 'store_slug', 'store_user_id', 'term', 'effective_annual_rate', 'lbl', 'ocurred_on'], 'custom_attributes': {}}, 'ProspectLoanAcceptedByBankingLicensePartner': {'stage': 'legacy_or_non_origination', 'direct_attributes': ['application_id', 'client_id', 'loan_id', 'origination_date', 'approved_amount', 'ally_slug', 'store_slug', 'store_user_id', 'term', 'effective_annual_rate', 'lbl', 'ocurred_on'], 'custom_attributes': {}}}
events_keys: ['LoanAcceptedByBankingLicensePartnerBR', 'LoanAcceptedWasNotSentToBankingLicensePartnerBR', 'ProspectLoanAcceptedByBankingLicensePartner']
