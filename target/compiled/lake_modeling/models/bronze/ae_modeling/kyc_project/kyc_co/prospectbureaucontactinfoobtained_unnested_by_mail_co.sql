...
WITH select_explode AS (
    SELECT
        ...
        EXPLODE(json_tmp.communications.mails) AS item
        -- CUSTOM ATTRIBUTES
...
SELECT
    ...
    application_id,
    client_id,
    'json_tmp.communications.mails._ARRAY_' AS array_parent_path,
    ocurred_on,
    ocurred_on_date,
    ingested_at,
    updated_at,
    ...
    -- ITEM INNER FIELDS
    item.address AS bureau_mail_address,
    TO_TIMESTAMP(item.firstReport) AS bureau_mail_firstReport,
    TO_TIMESTAMP(item.lastReport) AS bureau_mail_lastReport,
    item.order AS bureau_mail_order,
    item.timesReported AS bureau_mail_timesReported
